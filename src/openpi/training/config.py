...
class LeRobotAlohaDataConfig(DataConfigFactory):
    ...
    # traing_config.data.create 的功能是 datatransform
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
...
@dataclasses.dataclass(frozen=True)
class TrainConfig:
    # Name of the config. Must be unique. Will be used to reference this config. 配置名称。必须是唯一的。将用于引用此配置。
    name: tyro.conf.Suppress[str]
    # Project name.
    project_name: str = "openpi"
    # Experiment name. Will be used to name the metadata and checkpoint directories.实验名称。将用于命名元数据和检查点目录。
    exp_name: str = tyro.MISSING

    # Defines the model config. Some attributes (action_dim, action_horizon, and max_token_len) are shared by all models
    # -- see BaseModelConfig. Specific model implementations (e.g., Pi0Config) inherit from BaseModelConfig and may
    # define additional attributes.
    # 定义模型配置。所有模型共享一些属性（action_dim、action_horizon 和 max_token_len）--参见 BaseModelConfig。
    # 特定的模型实现（例如，Pi0Config）继承自 BaseModelConfig 并可能定义其他属性。
    ...
    # Determines the data to be trained on. 决定训练的数据
    ...
    save_interval: int = 1000  # 1000 步保留，到 5000 步删除
    # If set, any existing checkpoints matching step % keep_period == 0 will not be deleted.
    keep_period: int | None = 5000  # keep_period 可以用来保留检查点
